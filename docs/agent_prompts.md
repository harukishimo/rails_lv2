...
- 実装PRでは `.github/PULL_REQUEST_TEMPLATE.md` の `Evidence Matrix` を必ず埋め、要件/評価基準、証拠、確認方法、残リスクを対応させる
...
| Implementation Looper A | 実装時 | 中心ドメイン・高リスクIssueを実装する | PR、Loop Report |
| Implementation Looper B | 必要時のみ | 低リスク・独立Issueを並列実装する | PR、Loop Report |
| Test / QA Agent | 実装後/PR前 | テスト、lint、品質観点を確認する | テスト結果、修正Issue |
| Evidence Collector | PR後/実装後 | Issue、PR、テスト、コードパスを証跡化する | 証跡一覧 |
...
- 高リスクIssueではLoop Report/PR本文にリスクと判断内容を残させる
...
- 通常の人間確認はPRレビューへ回す。
- PR作成時はEvidence Matrixが埋まっていることを確認する。
- PRレビュー前に判断が必要な場合のみ `human-review` を提案する。
